...
def create_training_vs_validation_loss_curve(
    training_data, validation_data, title, xlabel, ylabel, output_image_name
):
    ...
    for graph_subject, graph_data in graph_lines.items():
        ...
        plt.plot(
            x_items, y_items, linewidth=1.5, linestyle="--", label=f"{graph_subject}"
        )
...
def generate_confusion_matrix(gt, pred, classes, checkpoint, result_folder, i=""):
    ...
    commat = pd.DataFrame(cm, index=classes, columns=classes)
    plt.figure()
...
def generate_precision_recall_graph(cm, classes, checkpoint, result_folder, i=None):
    ...
    pre_rec = pd.DataFrame(
        {"classes": classes, "precision": precision, "recall": recall}
    )
    ...
    mplcyberpunk.add_glow_effects()
    plt.title(
        f"Precison {round(total_precision*100, 2)}%   Recall {round(total_recall*100, 2)}% of {checkpoint}"
    )
...
def generate_barplot_for_one_metric(
    metric_dict, metric_name, experiment, result_folder
):
    plt.figure(figsize=(12, 8))
    metric_df = pd.DataFrame.from_dict(metric_dict, orient="index").reset_index()
    metric_df.columns = ["checkpoints", "value"]
    ax = sns.barplot(
        x="checkpoints", y="value", data=metric_df, hue="checkpoints", palette="mako"
    )
    for patch in ax.patches:
        ax.text(
            patch.get_x() + patch.get_width() / 2.0,
    ...
    ax.set_xticklabels(
        labels=[
            checkpoint.split(".")[0] for checkpoint in metric_df["checkpoints"].tolist()
        ],
        rotation=90,
    )
...
def evaluate_models(config_path):
    ...
    images_path = [
        image_path
        for image_path in os.listdir(test_set_path)
        if image_path.endswith(".jpg")
    ]
    labels = {
        image_path: load_labels_from_df(test_set_csv, image_path, config["classes"])
    ...
    for value in labels.values():
        if not isinstance(value, tuple):
            classwise_images[value] = (
                classwise_images[value] + 1
                if value in list(classwise_images.keys())
                else 1
            )
        else:
            for i in range(2):
                classwise_images[value[i]] = (
                    classwise_images[value[i]] + 1
                    if value[i] in list(classwise_images.keys())
                    else 1
                )
    accurates = {}
    precisions = {}
    ...
    for model_path in os.listdir(models_path):
        ...
        for image_name in images_path:
            ...
            (
                pred_probs.append(
                    [
                        probabilities[list(config["classes"].keys())[1]]
                        .squeeze()
                        .tolist(),
                        probabilities[list(config["classes"].keys())[0]]
                        .squeeze()
                        .tolist(),
                    ]
                )
                if len(probabilities.keys()) == 2
                else pred_probs.append(
            ...
            (
                gt.append(
                    [
                        config["classes"][list(config["classes"].keys())[1]].index(
                            label[1]
                        ),
                ...
                if len(list(config["classes"].keys())) == 2
                else gt.append(
        ...
        cm = generate_confusion_matrix(
            gt,
            preds,
            config["classes"][list(config["classes"].keys())[0]],
            model_path,
            result_folder,
            "",
        )

        precision, recall, f1 = generate_precision_recall_graph(
            cm,
            config["classes"][list(config["classes"].keys())[0]],
            model_path,
            result_folder,
    ...
    for key, value in accurates.items():
        ...
        for label_class, class_value in classwise_accurates[key].items():
            classwise_accuracies[key][label_class] = float(
                classwise_accurates[key][label_class] / classwise_images[label_class]
            )
    # Generate overall precision, recall and f1 graph
    generate_barplot_for_one_metric(
        precisions,
